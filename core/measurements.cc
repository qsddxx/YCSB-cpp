...
namespace ycsbc
{
  ...
  void BasicMeasurements::Report(Operation op, uint64_t latency)
  {
    ...
    while (prev_min > latency && !latency_min_[op].compare_exchange_weak(prev_min, latency, std::memory_order_relaxed))
      ;
    uint64_t prev_max = latency_max_[op].load(std::memory_order_relaxed);
    while (prev_max < latency && !latency_max_[op].compare_exchange_weak(prev_max, latency, std::memory_order_relaxed))
      ;
  }
  ...
  std::string BasicMeasurements::GetStatusMsg()
  {
    ...
    msg_stream << " Max num task in queue this peroid=" << undo_task_num[0] << " Min num task in queue this peroid=" << undo_task_num[1];
  ...
  HdrHistogramMeasurements::HdrHistogramMeasurements()
  {
    ...
    if (hdr_init(10, 100LL * 1000 * 1000 * 1000, 3, &his_task_num[0]) != 0)
    {
      utils::Exception("hdr init failed");
    }
  }
  ...
  std::string HdrHistogramMeasurements::GetStatusMsg()
  {
    ...
    msg_stream << "[ "
              << " Max num task in queue this peroid=" << hdr_max(his_task_num[0])
              << " Min num task in queue this peroid=" << hdr_min(his_task_num[0])
              << " Avg num task in queue this peroid=" <<hdr_mean(his_task_num[0])
              << " 90=" << hdr_value_at_percentile(his_task_num[0], 90)
              << " 99=" << hdr_value_at_percentile(his_task_num[0], 99)
              << " 99.9=" << hdr_value_at_percentile(his_task_num[0], 99.9)
              << " 99.99=" << hdr_value_at_percentile(his_task_num[0], 99.99)
              <<"]";
    //undo_task_num[0].store(0);
    //undo_task_num[1].store(std::numeric_limits<uint64_t>::max());
