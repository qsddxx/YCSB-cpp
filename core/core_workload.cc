...
namespace ycsbc
{
  ...
  void CoreWorkload::Init(const utils::Properties &p)
  {
    ...
    // record_count_longlast=record_count_;
    ...
    std::shared_ptr<DiscreteGenerator<Operation>> new_chooser = std::make_shared<DiscreteGenerator<Operation>>();
    ...
    {
      key_chooser_.store(std::make_shared<UniformGenerator>(0, record_count_ - 1));
    }
    else if (request_dist == "zipfian")
    {
      ...
      if (p.ContainsKey(ZIPFIAN_CONST_PROPERTY))
      {
        ...
        key_chooser_.store(std::make_shared<ScrambledZipfianGenerator>(0, record_count_ + new_keys - 1, zipfian_const));
      }
      else
      {
        key_chooser_.store(std::make_shared<ScrambledZipfianGenerator>((record_count_ + new_keys)));
      }
    }
    else if (request_dist == "latest")
    {
      key_chooser_.store(std::make_shared<SkewedLatestGenerator>(*transaction_insert_key_sequence_));
    }
    else if (request_dist == "multizip")
    {
      op_count_longlast = std::stoi(p.GetProperty(OPERATION_COUNT_PROPERTY));
      new_keys_longlast = (int)(op_count_longlast * insert_proportion * 2); // a fudge factor
      multi_zipfian = true;
      if (p.ContainsKey(ZIPFIAN_CONST_PROPERTY))
      {
        double zipfian_const = std::stod(p.GetProperty(ZIPFIAN_CONST_PROPERTY));
        key_chooser_.store(std::make_shared<AutoHotZoneSelector>(0, record_count_ + new_keys_longlast - 1, 0, 0, zipfian_const, zipfian_const));
      }
      else
      {
        key_chooser_.store(std::make_shared<AutoHotZoneSelector>(0, record_count_ + new_keys_longlast - 1));
      }
    }
    else
    {
  ...
  uint64_t CoreWorkload::NextTransactionKeyNum(OperationType op_type)
  {
    ...
    do
    {
      if (multi_zipfian)
      {
        key_num = key_chooser_.load()->Next(op_type);
      }
      else
      {
        key_num = key_chooser_.load()->Next();
      }
  ...
  void CoreWorkload::UpdateKeyChooser(double read_pos, double update_pos, double read_theta, double update_theta)
  {
    key_chooser_.store(std::make_shared<AutoHotZoneSelector>(0, record_count_ + new_keys_longlast - 1, read_pos, update_pos, read_theta, update_theta));
  }
} // ycsbc
