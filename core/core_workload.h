...
namespace ycsbc
{
  ...
  public:
    ...
    void UpdateKeyChooser(double read_pos = 0, double update_pos = 0, double read_theta = 0, double update_theta = 0);
  ...
  protected:
    ...
    std::atomic<std::shared_ptr<DiscreteGenerator<Operation>>> op_chooser_;
    // DiscreteGenerator<Operation> op_chooser_;
    std::atomic<std::shared_ptr<Generator<uint64_t>>> key_chooser_; // transaction key gen
    ...
    int new_keys_longlast = 0;
    int op_count_longlast = 0;
  };
